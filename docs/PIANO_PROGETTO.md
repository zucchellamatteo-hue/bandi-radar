...
**Aggiornamento del 25-26/09** (`docs/ricerche/2026-09-25_fonti_difficili.md`, PR #24): delle 44 fonti non verdi, **25 recuperate** (API interna comune di myPortal della Regione Veneto, feed per tipo di notizia dei Comuni su WordPress, due difetti dei lettori corretti), **8 escluse** (una è diventata un sito di casinò, altre non pubblicano bandi), **12 bloccate sull'indirizzo del server** (Lodi, Finlombarda www, Campobasso, Sviluppo Basilicata, Latina, Rieti, Viterbo, Catania, Potenza, Sistema Puglia...): rifiutano anche robots.txt e non hanno IPv6, mentre dal PC di Matteo si aprono. Un'uscita da un indirizzo italiano (proxy o piccolo server in Italia) resta **da valutare**; intanto molte sono coperte da incentivi.gov.it. In plancia 255 fonti verdi su 287. Una fonte che risponde ma da cui non si è mai letto nulla ora ha un motivo suo in plancia ("controllare la voce del registro"), distinto da "nessuna novità".
...
| 3 Schede | **24/09; 07/10–11/10** | fatta il 24/09 la parte senza IA (branch `claude/fase3-senza-ia`): tabelle `bandi`, `allegati`, `smistamenti`; smistamento a regole (`app.schede.smista`, parole in `app/schede/regole_smistamento.yaml`); scaricamento di allegati e FAQ con testo dei PDF (`app.schede.allegati`, volume `allegati`); nel catalogo esito dello smistamento, filtro e correzione a mano; pagina dell'annuncio con gli allegati; formato della scheda (`docs/SCHEDA_BANDO.md`) e prompt per Sonnet (`app/schede/prompt_scheda.md`). Il 25/09 prova dei modelli su dati reali, senza chiave API (Haiku su 90 annunci, Sonnet su 10 schede, revisione di Opus: `docs/ricerche/2026-09-25_prova_ia.md`): l'impianto regge, ma prima di collegare l'IA si cambia la struttura (§10, decise il 25/09). **Fatto il 25/09 (PR #18, #19, #20 e Parte 4)**: annuncio e bando separati con deduplica senza IA e pagina Doppioni (`app.schede.bandi`: 1.257 annunci rilevanti → 1.057 bandi, 116 dubbi); passo "pagina ufficiale" con regole nel registro e allegati per bando in ordine (`app.schede.pagina_ufficiale`: bando vero per 22 bandi su 29, tutti i 9 della prima prova); campi per l'abbinamento a valori controllati con tre stati, linee, completezza, stato calcolato ogni giorno (`app/schede/campi.py`, `docs/SCHEDA_BANDO.md`); prompt corretti e IA pronta ma spenta senza chiave (`app.schede.ia`: lotti da 20, controllo degli id, JSON vincolato, verifica automatica, Batch API, controllo preliminare con Haiku, tetto di spesa); seconda prova (`docs/ricerche/2026-09-25_prova_ia_2.md`: smistamento 100 risposte su 100, 0 errori gravi e 2 medi contro 3 e 5; schede senza invenzioni, media 3,5 su 5). **Fatto il 25/09 sera (PR #23, #24)**: i sei blocchi di dettagli chiesti da Matteo nella scheda (intensità e maggiorazioni, prestito e garanzie, vincoli sulle spese, esclusioni, obblighi, domanda); 25 fonti difficili recuperate. **Fatto il 26/09** (`docs/ricerche/2026-09-26_schede_in_produzione.md`): catena senza IA accesa in produzione (deduplica, pagina ufficiale, allegati per tutti i bandi); **schede dell'arretrato compilate nella sessione di Claude Code** su richiesta di Matteo, con gli stessi prompt e gli stessi controlli dell'API (vedi §10, decise il 26/09). Poi, con la chiave API Anthropic con tetto di spesa: Haiku per i "da rivedere", Sonnet per le schede, costi nella plancia, controllo di 30 schede da parte di Matteo |
...
**Decise il 25/09/2026 (sessione sulla struttura delle schede, `docs/sessioni/2026-09-25_struttura_schede.md`):**

| Tema | Decisione |
|---|---|
| Doppioni | Chiavi in ordine: codice ufficiale, indirizzo della pagina ripulito (un indirizzo ripetuto nella stessa fonte non conta), stesso ente + edizione + titolo. Due enti diversi dello stesso tipo (due Camere) sono doppioni solo con titolo identico. I dubbi restano senza bando finché non decide Matteo (pagina Doppioni), poi Haiku. Le scelte di Matteo non si toccano mai. |
| Pagina ufficiale | Regole per fonte nel registro (`pagina_ufficiale`: `campo`, `cerca`, `segui_link`, `escludi`, `sostituisci`, `documenti: plone_api`), scritte solo dopo averle verificate su un caso vero. Pagine di login, vuote o di solo menu non valgono. |
| Allegati | Si scaricano dalla pagina ufficiale del bando e hanno una categoria. Alla scheda: bando, pagina, FAQ, decreto più recente, il resto; la modulistica mai; si taglia dal fondo. |
| Campi di abbinamento | Valori ammessi in `app/schede/campi.py`, un solo posto. `ente` = chi finanzia, `gestore` a parte. Tre stati per ogni vincolo; `completezza` della scheda; `linee` facoltative. Lo stato del bando lo calcola il sistema ogni giorno. |
| Profilo del cliente | Forma giuridica e impresa femminile/giovanile si calcolano sul lato di Matteo (ragione sociale e codici fiscali dei soci restano lì); a Bandi Radar arriva solo il risultato. Il database `leadgen` contiene prospect, non clienti: da decidere quali imprese profilare nel pilota (`docs/RICHIESTA_SCHEMA_ANAGRAFICHE.md`). |
| IA | SDK ufficiale `anthropic`; modelli `claude-haiku-4-5` (smistamento, controllo preliminare) e `claude-sonnet-5` (schede); spenta senza `ANTHROPIC_API_KEY`; tetto di spesa anche nel programma (`IA_TETTO_MESE_USD`, 30 $); arretrato con la Batch API. |

**Decise il 25-26/09/2026 (PR #18–#24 unite in `main`, schede in produzione):**

| Tema | Decisione |
|---|---|
| Dettagli della scheda | Chiesti da Matteo il 25/09: oltre ai campi di abbinamento, sei blocchi per dire a un cliente se e quanto conviene un bando (`intensita`, `finanziamento`, `vincoli_spese`, `esclusioni`, `obblighi`, `domanda`), in `app/schede/campi.py` e `docs/SCHEDA_BANDO.md`. |
