...
**Open data**: una verifica del 23/09 (`docs/ricerche/2026-09-23_open_data_comuni.md`) mostra che i portali open data di Comuni e Regioni non contengono i bandi aperti, ma solo gare e sovvenzioni già concesse. Eccezioni che entrano nel registro: Regione Basilicata (elenco avvisi e bandi in JSON con scadenza e destinatari, da verificare dal server), calendari FESR/FSE+/FEASR della Provincia di Trento (CSV). Il catalogo nazionale dati.gov.it si osserva una volta al mese per scoprire nuovi dataset di bandi. I dati di contesto (perimetri dei Distretti del Commercio di Milano, aree ammissibili Emilia-Romagna, elenchi beneficiari) entrano in una categoria a parte del registro, per il matching e per il de minimis.
