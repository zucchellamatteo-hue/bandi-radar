...
| Dominio | **qiaro.it** (con qiaro.net che reindirizza), di proprietà di Matteo su GoDaddy: nessun passaggio da Sergio per il DNS. Non serve fino alla Fase 4 (plancia raggiunta via IP). Dalla Fase 5: `bandi.qiaro.it` per il cruscotto dei clienti, `radar.qiaro.it` per la plancia di Matteo, email inviate da un sottodominio dedicato (`bandi.qiaro.it`) registrato su Resend, così SPF e DKIM non interferiscono con eventuali invii di C2C dallo stesso dominio. Unica verifica con Sergio: se i nameserver di qiaro.it sono ancora quelli di GoDaddy o se li ha spostati (es. Cloudflare); i record si inseriscono dove stanno i nameserver. |
